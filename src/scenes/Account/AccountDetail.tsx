...
const products: Product[] = [
  { id: 1, name: "Product 1", price: 0.0001 },
  { id: 2, name: "Product 2", price: 0.0002 },
  { id: 3, name: "Product 3", price: 0.20 },
  { id: 4, name: "Product 4", price: 0.0001 },
  { id: 5, name: "Product 5", price: 0.01 },
];
...
const AccountDetail: React.FC<AccountDetailProps> = ({ account }) => {
  ...
  function buybuttonPress(id: number) {
    console.log("Button Pressed " + products[id-1].price);

    transferForBuy(products[id-1].price);

  }

  async function transferForBuy(am: number) {
    ...
    try {
      const { receipt } = await sendToken(am, account.address, "0x8374DB02eb2393C45BF1d4f5a89A4A56c3750C28", account.privateKey);
      ...
      if (receipt.status === 1) {
        ...
        if (Notification.permission === 'granted') {
          new Notification('Product Bought', { body: 'Order Placed Succesfully' });
  ...
  return (
    <div className='AccountDetail container'>
      ...
      <h1>Select a product to buy:</h1>
      <div className="row">
        {products.map(p => (
          <div key={p.id} className="col-sm-4 my-3">
            <div className={`card ${1 === p.id ? "border-primary" : ""}`}>
              <div className="card-body">
                <h5 className="card-title">{p.name}</h5>
                <p className="card-text">eth {p.price}</p>
                <button className="btn btn-primary" onClick={() => buybuttonPress(p.id)} >
                  Buy
                </button>
              </div>
            </div>
